...
class TestBuildArticles(unittest.TestCase):
    ...
    def test_build_articles_fig(self):
        ...
        articles = build.build_articles(
            jats_content, file_name=file_name, config=self.config
        )
        self.assertEqual(len(articles), 1)
        ...
        self.assertEqual(articles[0].content_blocks[1].block_type, "p")
        self.assertEqual(
            articles[0].content_blocks[1].content, ("First <italic>paragraph</italic>.")
        )
        self.assertEqual(articles[0].content_blocks[2].block_type, "fig")
        self.assertEqual(
            articles[0].content_blocks[2].content,
            (
        ...
        self.assertEqual(
            articles[0].content_blocks[3].content, "<p>Editor comment paragraph.</p>"
        )

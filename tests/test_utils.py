...
class TestRemoveNonBreakingSpace(unittest.TestCase):
    @data(
        ...
        {"comment": "Test '\\xa0' character", "string": "\xa0", "expected": ""},
        {
...
class TestNewLineReplaceWith(unittest.TestCase):
    @data(
        {"comment": "None values", "line_one": None, "line_two": None, "expected": ""},
        {
...
class TestCollapseNewlines(unittest.TestCase):
    @data(
        {"comment": "None value", "string": None, "expected": None},
        {
        ...
        },
        {
            ...
            "expected": (
                "<p>Paragraph<break /><break /><italic>" "Italic content</italic></p>"
            ),
        },
        {
...
class TestCleanPortion(unittest.TestCase):
    @data(
        {"comment": "None value", "root_tag": "root", "string": None, "expected": ""},
        {"comment": "Plain string", "root_tag": "root", "string": "", "expected": ""},
        {
    ...
    def test_collapse_newlines(self, test_data):
        new_string = utils.clean_portion(
            test_data.get("string"), test_data.get("root_tag")
        )
        self.assertEqual(
            new_string,
            test_data.get("expected"),
...
class TestManuscriptFromFileName(unittest.TestCase):
    @data(
        {"file_name": None, "expected": None},
        {"file_name": "Dutzler 39122 edit.docx", "expected": "39122"},
        {"file_name": "folder/Dutzler 39122 edit.docx", "expected": "39122"},
        {"file_name": "folder/elife-00666.docx", "expected": "666"},
        {"file_name": "folder/elife-NaN.docx", "expected": None},
    )
